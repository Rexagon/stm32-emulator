...
auto HexView::fullSize() const -> QSize
{
    if (!m_data.has_value()) {
        return QSize{0, 0};
    }
...
void HexView::resetSelection(int position)
{
...
void HexView::setSelection(int position)
{
    position = std::max(0, position);
...
void HexView::setCursorPosition(int position)
{
    position = std::max(0, position);
...
auto HexView::cursorPosition(const QPoint& position) -> int
{
    auto result = -1;
